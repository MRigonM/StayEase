...
const Settings = () => {
      ...
      try {
        ...
        if (userData) {
          ...
          if (nameParts.length === 1) {
            ...
            middlName = nameParts.slice(1, -1).join(" ");
          }
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      if (res.data.isSuccess) {
        // show modal to enter confirmation code
  ...
  const handleConfirmUpdate = async () => {
    try {
      const userId = user.id || user.Id;
      const res = await api.post("/Users/ConfirmUpdate", {
        id: userId,
      ...
      if (res.data.isSuccess) {
        ...
        setError(res.data.message || "Invalid confirmation code.");
      }
    } catch (err) {
      console.error("Confirm update error:", err);
      setError("Error confirming update.");
    }
  };
        ...
        {showConfirmModal && (
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
              <div className="bg-white p-6 rounded-xl shadow-xl max-w-sm w-full">
                <h2 className="text-xl font-bold mb-4">Enter Confirmation Code</h2>
                <input
                    type="text"
                ...
                <div className="flex gap-3">
                  <button
                      ...
                      className="flex-1 bg-logoColor text-white py-2 rounded-lg"
                  >
                    Confirm
                  </button>
                  <button
